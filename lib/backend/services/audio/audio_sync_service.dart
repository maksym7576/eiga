...
class AudioSyncService {
  ...
  AudioSyncService({required this.ffmpegService});
  ...
  Future<int> _getVideoDuration(String videoPath) async {
    ...
    try {
      final duration = await ffmpegService.getDuration(videoPath);
      if (duration != null) {
        _cachedVideoDuration = duration;
        return duration;
      }
    } catch (e) {
  ...
  Future<List<double>?> _extractAndGenerateVoiceMap(String videoPath, _SyncSegment seg, VadIterator iterator) async {
    ...
    try {
      ...
      final success = await ffmpegService.extractRawAudio(
        inputPath: videoPath, 
        outputPath: audioPath, 
        startSeconds: seg.startS, 
