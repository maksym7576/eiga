...
class ShikimoriSearchSource implements SearchSource<UnifiedMetadataDTO, void> {
  @override String get key => SearchSourceKeys.shikimori;
  @override String get title => 'Shikimori Search';
  @override String get searchHint => 'Search Shikimori...';
  ...
  Future<List<UnifiedMetadataDTO>> search(String query, Map<String, dynamic> filters, dynamic ref) async {
    ...
    if (results.isNotEmpty) {
      ...
      for (final res in results) if (res.shikimoriId != null) cache[res.shikimoriId!] = res;
  ...
  @override
  Future<List<dynamic>> fetchNextPage(String query, int page, Map<String, dynamic> filters, dynamic ref) async {
    final service = ref.read(shikimoriServiceProvider);
    final results = await service.searchAnime(query, page: page);
    if (results.isNotEmpty) {
      final cache = Map<int, dynamic>.from(ref.read(searchMetadataProvider(key)));
      for (final res in results) if (res is UnifiedMetadataDTO && res.shikimoriId != null) cache[res.shikimoriId!] = res;
      ref.read(searchMetadataProvider(key).notifier).state = cache;
    }
    return results;
  }
  ...
  @override String entryId(UnifiedMetadataDTO entry) => entry.shikimoriId?.toString() ?? entry.sourceId;
