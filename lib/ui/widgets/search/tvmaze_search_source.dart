...
class TVmazeSearchSource implements SearchSource<UnifiedMetadataDTO, void> {
  @override String get key => SearchSourceKeys.tvmaze;
  @override String get title => 'TVmaze Search';
  @override String get searchHint => 'Search for a show...';
  @override bool get hasFileStage => false;
  @override Map<String, dynamic> get defaultFilters => {};
  ...
  Future<List<UnifiedMetadataDTO>> search(String query, Map<String, dynamic> filters, dynamic ref) async {
    ...
    if (results.isNotEmpty) Future.microtask(() => _fetchMetadataForRange(results, 0, results.length, ref));
    return results;
  }
  ...
  Future<void> _fetchMetadataForRange(List<UnifiedMetadataDTO> results, int start, int end, dynamic ref) async {
    ...
    final missingEntries = range.where((e) => e.episodes == null && e.tvmazeId != null && !currentMetadata.containsKey(e.tvmazeId)).toList();
    ...
    for (int i = 0; i < missingEntries.length; i += 5) {
      ...
      final List<Future<void>> tasks = batch.map((entry) async {
        try {
          ...
          batchMetadata[entry.tvmazeId!] = fullShow ?? const NoMetadataDTO();
        } catch (e) { batchMetadata[entry.tvmazeId!] = const NoMetadataDTO(); }
      }).toList();
      await Future.wait(tasks);
      if (batchMetadata.isNotEmpty) ref.read(searchMetadataProvider(key).notifier).state = {...ref.read(searchMetadataProvider(key)), ...batchMetadata};
      if (i + 5 < missingEntries.length) await Future.delayed(const Duration(milliseconds: 200));
    }
  }

  @override Future<List<dynamic>> fetchNextPage(String query, int page, Map<String, dynamic> filters, dynamic ref) async => [];
  ...
  Future<String> resolve(dynamic selected, dynamic ref) async {
    ...
    if (entry.tvmazeId != null) {
      ...
      if (currentSelected != null && currentSelected is UnifiedMetadataDTO && entryId(currentSelected) == entry.tvmazeId.toString()) {
        ...
        if (fullData != null) ref.read(selectedEntryProvider(key).notifier).state = fullData;
      }
  ...
  @override Widget buildFilterBar(BuildContext context, WidgetRef ref) => const SizedBox.shrink();
  @override Future<List<void>> getFiles(UnifiedMetadataDTO entry, Map<String, dynamic> filters, dynamic ref) async => [];
  @override Widget buildFileCard(void file, bool isActive, VoidCallback onTap) => const SizedBox.shrink();
  @override String entryId(UnifiedMetadataDTO entry) => entry.tvmazeId?.toString() ?? entry.sourceId;
  @override String fileId(void file) => '';
